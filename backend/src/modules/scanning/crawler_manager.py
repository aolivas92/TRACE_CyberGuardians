...
class crawler_manager:
    ...
    async def crawl_recursive(self, url: str, depth_remaining: int, parent_url: str = None) -> None:
        ...
        try:
            ...
            if depth_remaining == self.config.get("depth"):
                with open("src/database/crawler_results/raw_html.txt", "w", encoding="utf-8") as f:
                    f.write(raw_html)
    ...
    async def start_crawl(self) -> list:
        ...
        with open("src/database/crawler_results/crawler_table_data.json", "w", encoding="utf-8") as f:
