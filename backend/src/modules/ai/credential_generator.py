...
class Credential_Generator:
    ...
    def calculate_password_strength(self, password: str) -> str:
        ...
        try:
            ...
            warnings.warn(
                "Failed to establish connection in Ollama, will proceed with fallback scoring method"
            )
            if score > 0.7:
    ...
    def get_ai_hyperparameters(self) -> list[str]:
        ...
        if not self.hyperparameters:
            raise ValueError("No hyperparameters have been provided")
    ...
    def _load_wordlist(self, file_path: str) -> List[str]:
        ...
        try:
            ...
            raise PermissionError(
                f"Program does not have permission to operate on file: {pe}"
            )
        except IOError as ioe:
            raise IOError(f"Failed to perform i/o operations on the file: {ioe}")
        except Exception as e:
